...
{
	 SAMPLE	    **xpr, **xpi, **xqr, **xqi, *cr = NULL, *ci = NULL;
	 int	    i;
...
void pd_fft(float *buf, int npoints, int inverse)
{
  ...
  if (inverse)
	...
	for (i = npoints << 1, fp = buf; i--; fp++) *fp *= npoints;
	}
#ifdef UNUSED
{
  #define STEPME 1 /*(npoints==1024 ? 1 : 4)*/
  for (i = 0; i<npoints/2; i+=STEPME)
	{
	/* max of every 7 points of npoints=1024 fits in 80 columns */
	float t = buf[i];
	int j;
	...
	if (npoints>1024)
		{
		if (buf[i+1] > t) t = buf[i+1];;;;
		if (buf[i+2] > t) t = buf[i+2];;;;
		if (buf[i+3] > t) t = buf[i+3];;;;
		}
	*/
