...
{
    t_pd *x;
    if (!c) post("pd_new: apparently called before setup routine");
...
{
    t_class *c = *x;
    if (c->c_freemethod) (*c->c_freemethod)(x);
    if (c->c_patchable)
    {
...
{
    t_bindelem *e;
    for (e = x->b_list; e; e = e->e_next) typedmess(e->e_who, s, argc, argv);
}
...
void pd_unbind(t_pd *x, t_symbol *s)
{
    ...
    else error("%s: couldn't unbind", s->s_name);
}
...
/* stack for maintaining bindubgs for the #X symbol during nestable loads.
*/
...
{
    if (lastpopped) vmess(lastpopped, gensym("loadbang"), "");
    lastpopped = 0;
}
