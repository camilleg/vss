...
/* Return a pointer to the resulting tree.                   */
    ...
    tnew->key = i;
    tnew->size = 1 + node_size(tnew->left) + node_size(tnew->right);
    return tnew;
}
...
simplex *add_to_fg(simplex *s, void *) {
...
void visit_fg_i(   void (*v_fg)(Tree *, int, int),
	...
	if (t->fgs->mark!=vn) {
		t->fgs->mark = vn;
		if (t->key!=infinityPoint && !mo[site_num(t->key)]) boundaryc = 0; 
...
int visit_fg_i_far(void (*v_fg)(Tree *, int),
	...
	if (t->fgs->mark!=vn) {
		t->fgs->mark = vn;
		nb = (t->key==infinityPoint) || mo[site_num(t->key)];
...
void h_fg(Tree *t, int depth, int bad) {
	if (t->fgs->facets)
		(bad ? fg_hist_bad : fg_hist)[depth][t->fgs->facets->size]++;
}
