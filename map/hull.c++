...
int	rdim=0,	/* region dimension: (max) number of sites specifying region */
	cdim=0,	/* number of sites currently specifying region */
	site_size=0, /* size of malloc needed for a site */
	point_size=0;  /* size of malloc needed for a point */
...
/* make neighbor connections between newly created simplices incident to hull_p */
	...
	if (!s) return;
	assert(!s->peak.vert);
	assert(s->peak.simp->peak.vert==hull_p);
	int ret = (!op_vert(s,hull_p)->simp->peak.vert);
	assert(ret);
...
/*
 * hull_p lies outside flat containing previous sites;
 * make hull_p a vertex of every current simplex, and create some new simplices
 */
 ...
 * current hull, and sees(hull_p, s) */

	simplex *s;
	static simplex **st;
	static long ss = MAXDIM*8; //;;CG *8 so fewer realloc's below, they tend to dump core
	neighbor *sn;
	int i;
	long tms = 0;
