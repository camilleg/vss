...
int check_overshoot_f=0;


simplex *ch_root = NULL;
...
#define check_overshoot(x)							\
	{if (CHECK_OVERSHOOT && check_overshoot_f && ((x)>9e15))		\
		hullwarning(-20, overshot exact arithmetic)}			\
...
Coord	infinityPoint[10]={57.2,0,0,0,0}; /* point at infinity for vd; value not used */
...
/*
	if (!finite(v->sqa) || isnan(v->sqa))
		{
		printf("BOGUS BOGUS BOGUS \n\n\n");;;;
		}
	else
		printf("Ok.\n");
*/
	...
	if (j<10) {
		...
		if (max_scale<1) {
//			printf("%g %d %d huh?\n", v->sqa, finite(v->sqa) , isnan(v->sqa));
			printf("%g %d %g %g %d\n", 
				max_scale,
				exact_bits,
				labound,
			...
			hullwarning(-10, overshot exact arithmetic);
			max_scale = 1;
...
inline void trans(point z, point p, point q)
{
...
inline void lift(point z)
{
	if (vd)
		z[2*rdim-1] = z[rdim-1] = ldexp(Vec_dot_pdim(z,z), -DELIFT);
...
void get_basis_sede(simplex *s) {
	...
	if (vd && sn0->vert == infinityPoint && cdim >1) {
...
int out_of_flat(simplex *root, point p) {
	...
	get_basis_sede(root);
	if (vd && root->neigh[0].vert == infinityPoint) return 1;
...
int check_perps(simplex *s) {
	...
	for (i=1;i<cdim;i++) {
		y = s->neigh[i].vert;
		if (vd && y==infinityPoint) memcpy(b, infinity_basis, basis_s_size);
		else {trans(z,y,tt); lift(z);}
...
int sees(site p, simplex *s) {
	...
	tt = s->neigh[0].vert;
	if (vd) {
		if (p==infinityPoint) memcpy(b,infinity_basis,basis_s_size);
		else {trans(zz,p,tt); lift(zz);}
	} else trans(zz,p,tt);
	for (i=0;i<3;i++) {
...
double radsq(simplex *s) {
	...
	for (i=0,sn=s->neigh;i<cdim;i++,sn++)
		if (sn->vert == infinityPoint) return Huge;
...
simplex *zero_marks(simplex * s, void *) { s->mark = 0; return NULL; }

simplex *one_marks(simplex * s, void *) {s->mark = 1; return NULL;}

simplex *show_marks(simplex * s, void *) {printf("%d",s->mark); return NULL;}
...
int alph_test(simplex *s, int i, void *alphap) {
	...
	for (k=0;k<cdim;k++) if (s->neigh[k].vert==infinityPoint && k!=i) return 1;
	rs = radsq(s);
	rsi = radsq(si);
	...
	cdim--;
	get_basis_sede(s);
	reduce(&s->normal,infinityPoint,s,cdim);
	rsfi = radsq(s);
...
simplex *conv_facetv(simplex *s, void *) {
	int i;
	for (i=0;i<cdim;i++) if (s->neigh[i].vert==infinityPoint) {return s;}
	return NULL;
}

short mi[MAXPOINTS], mo[MAXPOINTS];

simplex *mark_points(simplex *s, void *) {
	int i, snum;
	neighbor *sn;

	for  (i=0,sn=s->neigh;i<cdim;i++,sn++) {
		if (sn->vert==infinityPoint) continue;
...
simplex* visit_outside_ashape(simplex *root, visit_func visit) {
...
double find_alpha(simplex *root) {
	...
	for (ah=0.f,i=0;i<pdim;i++)
		ah += (float)(maxs[i]-mins[i])*(maxs[i]-mins[i]);
	int ret = check_ashape(root,ah);
	assert(ret);
	for (i=0;i<17;i++) {
...
void vols(fg *f, Tree *t, basis_s* n, int depth) {
	...
	if (t->fgs->dist == 0) {
		...
		reduce(&nn, infinityPoint, s, depth);
		nnv = nn->vecs;
		if (t->key==infinityPoint || f->dist==Huge || NEARZERO(nnv[rdim-1]))
			t->fgs->dist = Huge;
		else
...
*/
	...
	if (!Huge) Huge = DBL_MAX; /*CG: this makes a warning: DBL_MAX*DBL_MAX; */
	...
	//;;;; exact_bits = 53; //;;;; (int)(DBL_MANT_DIG*log(FLT_RADIX)/log(2.));
