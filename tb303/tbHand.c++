...
tb303Hand::tb303Hand(tb303Alg * alg):
	VHandler( alg )
{ 
	// Don't call Retrigger() here, you'll get an unwanted pre-attack
	// *before* the BeginSound SetFoo bar's have been processed.
...
void tb303Hand::SetAttribute(IParam iParam, float z)
{
	if (iParam.FOnlyI())
		...
		case isetFreq:
			if (!CheckFreq(z))
				printf("tb303Handler got bogus frequency %f.\n", z);
			else
				getAlg()->setFreq(zFreq = z);
			break;
		case isetFilterCutoff:
			if (!CheckFilterCutoff(z))
				printf("tb303Handler got bogus FilterCutoff %f.\n", z);
			else
				getAlg()->setFilterCutoff(zFilterCutoff = z);
			break;
		case isetResonance:
			if (!CheckResonance(z))
				printf("tb303Handler got bogus Resonance %f.\n", z);
			else
				getAlg()->setResonance(zResonance = z);
			break;
		case isetEnvMod:
			if (!CheckEnvMod(z))
				printf("tb303Handler got bogus EnvMod %f.\n", z);
			else
				getAlg()->setEnvMod(zEnvMod = z);
			break;
		case isetEnvDecay:
			if (!CheckEnvDecay(z))
				printf("tb303Handler got bogus EnvDecay %f.\n", z);
			else
				getAlg()->setEnvDecay(zEnvDecay = z);
			break;
		default:
			printf("vss error: tb303Handler got bogus float-index %d.\n", iParam.i);
			}
		}
	else
		printf("vss error: tb303Handler got bogus element-of-float-array-index %d.\n", iParam.i);
}
