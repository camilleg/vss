/*	fft.c  --  real and complex forward and inverse fft's */
...
/* number of cached twidle factor arrays */
#define CMAX 8
...
static void maketwiddle(int n, int notinverse)
{
	...
	}
	{
			...
			if (c[ncount].twiddle)
				delete [] c[ncount].twiddle;
			c[ncount].twiddle = new float[2*n];
...
#define xGOOD
			...
			for(j=1,uNQmq = 0;j<mmax; j+= 2,uNQmq += NQmq)
			{
				...
				for(i=j,kp=&a[mmax+j], ap=&a[j],  kkp = &a[ mmax+j+istep]; i<=(N-istep); i += istep,
							ap += istep, kp+= istep, kkp+= istep)	
				{
					{
						/* complex multiply of a by u */
						tr -= tc;
...
*/
void realfft(int n, float *a, int notinverse);
#define TFAST
void realfft(int n, float *a, int notinverse)
{
...
{
	assignvec(n,rR, 2, r, rstride);
	kvec(0.0f,n,rR+1, 2);
	fftComplex( n, rR, TRUE);
}  
	...
	int i;
        res[0] = *r; r += istride;
         res[1] = 0.0f;
          ...
          {
             res[i*2] = *r;				 r += istride;
             res[i*2+1] = (*c);		 c += istride;
             res[(n-i)*2] = res[i*2];
             res[(n-i)*2+1] = -res[i*2+1];
           }

	fftComplex( n, res,  FALSE);
	assignvec(n,res, 1, res,2);
} 
extern void fftRealfast(int n, float *r)
{
	realfft( n/2, r, TRUE);
}  
